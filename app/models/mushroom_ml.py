...
class Mushroom_ml():
    ...
    def __init__(self,df, target, engine):
        """--------------------------------------------------------
...
#--------------------------------------------------------------------
    ...
    def score_model(self, sendmodel=DecisionTreeClassifier(), encoder=OneHotEncoder(handle_unknown='ignore'), display_bol=True):
        """--------------------------------------------------------
        Train a Model and Score Metrics. Add Monitoring parameters and launch save models

            Parameters
            Model, Encoder, Display_bol
    ...
    def feature_selection(self, list_val_features=[], sendmodel=DecisionTreeClassifier(), plot_df=False):
        ...
        results_test_models = []
        for col in liste:
            df_test = Mushroom_ml(self.df[['_class', col] + list_val_features], '_class', self.engine)
